package s4.B223364;  // ここは、かならず、自分の名前に変えよ。
import java.lang.*;
...
public class Frequencer implements FrequencerInterface{
    ...
    private int suffixCompare(int i, int j) {
        ...
        // ここにコードを記述せよ 
        //
        byte [] suffix_i={};//iの位置から始まる文字列
        byte [] suffix_j={};//jの位置から始まる文字列
        int n,num;
        //iの位置から始まる文字列
        for(n = i,num=0; n < mySpace.length; n++,num++){
            suffix_i[num] = mySpace[n];
        }
        //ｊの位置から始まる文字列
        for(n = j,num=0; n < mySpace.length; n++,num++){
            suffix_j[num] = mySpace[n];
        }

        //小さいほうを返す
        int result = 0;           
        for(n = 0;(n <= (suffix_i.length)) || (n <=(suffix_j.length));n++){
            //n番目の文字の大小の比較
            ...
            //全ての文字が同じだった場合は文字列の長さで比較
            if((n == (suffix_i.length)) || (n == (suffix_j.length))){
    ...
    public void setSpace(byte []space) { 
        ...
        int[] ini_suffix = {};
        int n,place;
        for(n = 0; n < mySpace.length; n++){
            for(place = n; place < mySpace.length; place++){
                suffix[n] = suffix[n] + mySpace[place];
            }
        }
        suffix = ini_suffix;
        Arrays.sort(suffix);
        for(n = 0;n < mySpace.length;n++){
            for(place = 0; place < mySpace.length; place++){
                if(ini_suffix[n] == suffix[place]){
                    suffixArray[n] = place;
                }
            }
        }
    ...
    private int targetCompare(int i, int j, int k) {
        ...
        // ここに比較のコードを書け 
        ////
        byte [] suffix_i={};//mySpaceのiの位置から始まる文字列
        byte [] target_j_k={};//myTargetのjの位置から始まりkの位置で終わる文字列
        int n,num;
        //iの位置から始まる文字列
        for(n = i,num=0; n < mySpace.length; n++,num++){
            suffix_i[num] = mySpace[n];
        }
        //ｊの位置から始まりkの位置で終わる文字列
        for(n = j,num=0; (n < mySpace.length) || (n <= k); n++,num++){
            target_j_k[num] = myTarget[n];
        }

        //小さいほうを返す
        int result = 0;           
        for(n = 0;n <=(target_j_k.length);n++){
            //先頭が同じ場合は0を返す．
            for(num = 0;num <= target_j_k.length;num++){
                if(suffix_i[num] != target_j_k[num]){
                break;
                }
            }
            if(num == target_j_k.length){
                result = 0;
                break;
            }
            //targetのほうが文字列が長い時は-1を返す．
            ...
            //文字の大小の比較
            if(suffix_i[n] > target_j_k[n]){
                result = 1;
            }
            else if(suffix_i[n] < target_j_k[n]){
                result = -1;
            }
        }
        //resultに格納されている値(比較結果)を返す
