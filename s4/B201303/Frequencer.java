package s4.B201303;  //~A~S~A~S~A~@~A~A~K~A~B~I~A~Z~@~A~G~H~F~A~P~M~I~M~A~I~A~H~B~H~@~B
import java.lang.*;
...
public class Frequencer implements FrequencerInterface{
	...
	//System.out.println(mySpace.length);
	    ...
	    if(i > mySpace.length - 1) {win = -1; break;}
	    else if(j > mySpace.length - 1) {win = 1; break;}
	    else if(mySpace[i] > mySpace[j]) {win = 1; break;}
	    else if(mySpace[i] < mySpace[j]) {win = -1; break;}
	    else{
		i += 1;
		j += 1;
	...
	for(int m = j; j < k; j++){
		...
		if(mySpace[i] > mySpace[j]) {win = 1; break;}
		else if(mySpace[i] < mySpace[j]) {win = -1; break;}
	    }
	    else{
		i += 1;
	...
	// Suppose target is set "Ho Ho Ho Ho"
        ...
        //int target_start = 0;
        int i = 0;
        for(i = 0; i < suffixArray.length - 1; i++){
	    //System.out.println(suffixArray[i]);
	    if(targetCompare(suffixArray[i], start, end) == 0) {break;}
	}
	//System.out.println(i);                                                                         
	...
	int i = 0;
        for(i = 0; i < suffixArray.length - 1; i++){
            //System.out.println(suffixArray[i]);
            if(targetCompare(suffixArray[i], start, end) == 1) {break;}
        }
        //System.out.println(i);
        return i; // この行は変更しなければならない、       
    }
    ...
    public static void main(String[] args) {
	    ...
	    frequencerObject.printSuffixArray();
	    int win = frequencerObject.suffixCompare(0, 1);
	    if(win == -1) {System.out.println("OK");} else {System.out.println("WRONG");}
	    win = frequencerObject.suffixCompare(0, 6);
	    if(win == 1){System.out.println("OK");} else {System.out.println("WRONG");}
	    win = frequencerObject.suffixCompare(6, 9);
	    if(win == -1){System.out.println("OK");} else {System.out.println("WRONG");}
	    win = frequencerObject.targetCompare(9, 0, 1);
	    if(win == 0) {System.out.println("OK");} else {System.out.println("WRONG");}
	    win = frequencerObject.targetCompare(10, 1, 2);
	    if(win == 1) {System.out.println("OK");} else {System.out.println("WRONG");}
	    win = frequencerObject.targetCompare(5, 0, 2);
	    if(win == -1) {System.out.println("OK");} else {System.out.println("WRONG");}
	    int start_target = frequencerObject.subByteStartIndex(3, 5);
	    if(start_target == 5) {System.out.println("OK");} else {System.out.println("WRONG");}
	    start_target = frequencerObject.subByteStartIndex(3, 6);
	    if(start_target == 6) {System.out.println("OK");} else {System.out.println("WRONG");}  
            int end_target = frequencerObject.subByteEndIndex(3, 5);
	    if(end_target == 7) {System.out.println("OK");} else {System.out.println("WRONG");}
	    end_target = frequencerObject.subByteEndIndex(1, 2);
            if(end_target == 9) {System.out.println("OK");} else {System.out.println("WRONG");}
